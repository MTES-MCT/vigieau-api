...
export class ZonesService {
  ...
  searchZonesByLonLat({lon, lat}, allowMultiple = false) {
    ...
    let restrictions = zones.map(z => z.restrictions).flat().concat(communes.map(c => c.restrictions).flat());
    // remove empty values
  ...
  async loadAllZones() {
    ...
    await Promise.all(zones.map(async (zone) => {
      const z = await this.zoneAlerteRepository.findOne({
        ...
        relations: ['restrictions', 'restrictions.arreteRestriction', 'restrictions.usages']
      })
